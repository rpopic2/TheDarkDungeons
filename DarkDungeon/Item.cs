public record Item(string name, ItemType itemType, IBehaviour[] skills)
{
    ...
    public static readonly Item sword = new("(검)", ItemType.Equip, new Skill[] {
        new("베기", TokenType.Offence, StatName.Sol, "은 칼을 휘둘러 앞을 베었다.", (f)=>f.Throw(1)),
        new("칼로막기", TokenType.Defence, StatName.Lun, "은 칼로 막기 자세를 취했다.", (f)=>f.Dodge())
        });
    public static readonly Item holySword = new("(광란의 신성검)", ItemType.Equip, new Skill[] {
        new("베기", TokenType.Offence, StatName.Sol, "은 칼을 휘둘러 앞을 베었다.", (f)=>f.Throw(1)),
        new("광란의기도", TokenType.Charge, StatName.Con, "은 미친 듯이 기도하였고 칼이 빛나기 시작했다.", (f)=>{})
        });
    public static readonly Item staff = new("(지팡이)", ItemType.Equip, new Skill[] {
        new("휘두르기", TokenType.Offence, StatName.Sol, "은 지팡이를 휘둘렀다.", (f)=>f.Throw(1)),
        new("별빛부름", TokenType.Charge, StatName.Con, "은 신비한 별빛을 불러내어 지팡이를 휘감았다.", (f)=>{})
        });
    public static readonly Item dagger = new("(단검)", ItemType.Equip, new Skill[] {
        new("찌르기", TokenType.Offence, StatName.Sol, "은 단검으로 적을 찔렀다.", (f)=>f.Throw(1)),
